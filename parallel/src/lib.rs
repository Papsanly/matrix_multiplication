...
pub fn multiply<T: NumAssign + Clone + Copy + Send + Sync + Debug>(
    ...
    let multiply_sub_matrix = |thread_idx| {
        let from_row = thread_idx * res.rows / num_threads.min(res.rows);
        let to_row = (thread_idx + 1) * res.rows / num_threads.min(res.rows);
    ...
    thread::scope(|s| {
        for thread_idx in 0..num_threads.min(res.rows) {
